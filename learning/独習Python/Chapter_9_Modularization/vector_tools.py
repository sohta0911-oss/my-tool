# vector_tools.py

class Vector:
    """Represents a mathematical vector and provides basic operations.
    Attributes:
        components: A list of numerical values representing the vector.
    """
    
    def __init__(self, components):
        """Initializes the vector with components."""
        self.components = components # これが「インスタンス変数
        
    def l2_norm(self):
        """Calculates the L2 norm."""
        return (sum(x**2 for x in self.components))**(1/2)

    def inner_product(self, other):
        """Calculates the inner product with another vector.

        Args:
            other: Another vector instance to calculate the inner product with.

        Returns:
            The inner product value (float).

        Raises:
            ValueError: If the dimensions of the vectors do not match.
        """
        if len(self.components) != len(other.components):
            raise ValueError("Demension must much for inner product")
        return sum(x * y for x, y in zip(self.components, other.components))

    def similality(self, other, threshold=0.8):
        """Caluculate the cosine similalitu with another vector
        
        Args:
            other: Another Vector instance to caluculate the cosine similality with.
            threshold: Similality threshold to determine 'similar' or 'status'.

        Returns:
            A turple of (similality_score, status_label).
        """
        inner_product = self.inner_product(other)
        norm_1 = self.l2_norm()
        norm_2 = other.l2_norm()

        if norm_1 == 0 or norm_2 == 0:
            raise ValueError("Cannot calculate similaity with zero vector.")

        score = inner_product / (norm_1 * norm_2)
        if score > threshold:
            status = "similar"
        else:
            status = "not similar"

        return score, status





def calculate_inner_product(a,b):
    # 内積の定義
    if len(a) !=len(b):
        raise ValueError("ベクトルの次元が一致しません")
    
    inner_product = sum(x*y for x,y in zip(a,b))
    return inner_product


def calculate_l2_norm(a):
    # L2ノルムの定義
    return sum(x**2 for x in a)**0.5


def calculate_cosine_similarity(a,b, threshold=0.8):
    """
    Calculate the cosine similarity between two vectors.
    
    Args:
        a, b (list): Input vectors.
        threshold (float): Similarity threshold (default: 0.8).
        
    Returns:
        tuple: (similarity_score, status_label)
    """

    inner_product = calculate_inner_product(a,b)
    norm_a = calculate_l2_norm(a)
    norm_b = calculate_l2_norm(b)

    if norm_a == 0 or norm_b == 0: #0除算を防ぐ！
        raise ValueError("ベクトルのノルムがゼロです")
    
    cosine_similarity = inner_product / (norm_a * norm_b)

    if cosine_similarity > threshold:
        status = "similar"
    else:
        status = "not similar"
    
    return cosine_similarity, status